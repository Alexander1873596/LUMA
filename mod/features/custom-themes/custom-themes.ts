...
async function updateTheme() {
  const enabled = (await window.LumaCloudMod.getStorageValue("custom-themes/enabled")) === true ? true : false;
  const accent = await window.LumaCloudMod.getStorageValue("custom-themes/accent");
  const playerColorsReplaceEnabled =
    (await window.LumaCloudMod.getStorageValue("custom-themes/playerColorsReplace")) === false ? false : true;
  const disableVibeAnimation = await window.LumaCloudMod.getStorageValue("custom-themes/disableVibeAnimation");
  const disableExplicitMark = await window.LumaCloudMod.getStorageValue("custom-themes/disableExplicitMark");
  const hiddenMenuItems = (await window.LumaCloudMod.getStorageValue("custom-themes/hideMenuItems")) || [];
  ...
  if (enabled && accent) {
    ...
    if (playerColorsReplaceEnabled) {
      styleSheet.innerHTML += `
        ...
        {
          --player-average-color-background: var(--LumaCloudModPalette-200) !important;
        }
        .ym-dark-theme 
        ...
        div[data-test-id="FULLSCREEN_PLAYER_MODAL"]
        {
          --player-average-color-background: var(--LumaCloudModPalette-700) !important;
        }
        .ym-dark-theme div[class*="_averageColorBackground__"] {
          background: linear-gradient(var(--LumaCloudModPalette-200, var(--ym-background-color-secondary-enabled-blur)) 0, transparent 100%); 
        }
        .ym-light-theme div[class*="_averageColorBackground__"] {
          background: linear-gradient(var(--LumaCloudModPalette-light-300, var(--ym-background-color-secondary-enabled-blur)) 0, transparent 100%); 
        }
      `;
    }
  ...
  window.Worker = function (...args: any[]) {
    ...
    worker.postMessage = function (message, transfer) {
      if (message?.payload?.backgroundColor || message?.payload?.collectionHue) {
        if (!message?.payload?.isLumaCloudMod)
          setTimeout(() => {
            updateVibeBackgroundColor();
          }, 500);
      }

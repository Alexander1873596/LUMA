...
export function Downloader() {
  ...
  useEffect(() => {
    const loadDownloadPath = async () => {
      try {
        const savedPath = await window.LumaCloudMod.getStorageValue("downloadFolderPath");
        if (savedPath) {
  ...
  const saveDownloadPath = async (path: string | null) => {
    try {
      await window.LumaCloudMod.setStorageValue("downloadFolderPath", path);
  ...
  const handleSelectFolder = async () => {
    try {
      const result = await window.LumaCloudMod.selectDownloadFolder();
  ...
  const handleOpenFolder = async () => {
    ...
    try {
      const result = await window.LumaCloudMod.openFolder(downloadFolderPath);
      if (!result.success) {
  ...
  async function downloadTracks(trackIds: string[], quality: QualityEnum) {
    ...
    for (var i = 0; i < tracks.length; i++) {
      ...
      const downloadResult = await window.LumaCloudMod.downloadTrack(
        downloadInfo.value,
        tracks[i],
        downloadFolderPath || "",
      );

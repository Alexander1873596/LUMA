  ...
  useEffect(() => {
    (async () => {
      setDiscordRPCEnabled(
        (await window.LumaCloudMod.getStorageValue("discordRPC/enabled")) === false ? false : true,
      );
      setShowModButton(
        (await window.LumaCloudMod.getStorageValue("discordRPC/showModButton")) === false ? false : true,
      );
    })();
  }, []);
        ...
        <div className="flex items-center gap-3">
          <Switch
            ...
            onCheckedChange={(enabled) => {
              ...
              window.LumaCloudMod.setStorageValue("discordRPC/enabled", enabled);
            }}
          />
        ...
        <div className="flex items-center gap-3">
          <Switch
            ...
            onCheckedChange={(enabled) => {
              ...
              window.LumaCloudMod.setStorageValue("discordRPC/showModButton", enabled);
            }}
          />
          <Label htmlFor="discord-mod-button-toggle" className="cursor-pointer">
            Показывать кнопку LumaCloudMod
          </Label>
        </div>
      </div>
    </ExpandableCard>

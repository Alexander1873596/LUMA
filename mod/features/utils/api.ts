...
const headers = {
  "X-Yandex-Music-Client": "LumaCloudDesktopAppWindows/" + window.VERSION,
...
const LumaCloudClient: AxiosInstance = axios.create({
...
export async function getTrackUrl(trackId: string, quality: QualityEnum): Promise<Result<any, string>> {
  try {
    ...
    for (let i = 0; i < 10; i++) {
      const response = await LumaCloudClient.get(
...
export async function getPlaylistTracks(id: string): Promise<Result<string[], string>> {
  try {
    const response = await LumaCloudClient.get(`/playlist/${id}?resumeStream=false&richTracks=false`);
...
export async function getArtistTracks(id: string): Promise<Result<string[], string>> {
  try {
    const response = await LumaCloudClient.get(`/artists/${id}/track-ids?`);
...
export async function likeTrack(userId: number, trackId: string): Promise<Result<any, string>> {
  try {
    const response = await LumaCloudClient.post(
...
export async function getAccountInfo(): Promise<Result<{ uid: number }, string>> {
  try {
    const response = await LumaCloudClient.get("/account/about");
...
> {
  try {
    const response = await LumaCloudClient.get("/landing-blocks/likes-and-history");
...
> {
  try {
    const response = await LumaCloudClient.get("/account/settings");
...
export async function updateAccountSettings(
  ...
  try {
    const response = await LumaCloudClient.post(`/account/settings?${key}=${value}`);
...
export async function getAccountExperiments(): Promise<Result<any, string>> {
  try {
    const response = await LumaCloudClient.get("/account/experiments/details");

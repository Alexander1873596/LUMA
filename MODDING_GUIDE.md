...
`LumaCloudBetaMod` - это автоматический патчер, который берет актуальную стабильную сборку Яндекс Музыки, вносит изменения и собирает модифицированное приложение.
...
## Как добавлять свои моды в LumaCloudBetaMod
...
```bash
git clone https://github.com/Stephanzion/LumaCloudBetaMod.git
cd LumaCloudBetaMod
```
...
- запись состояния в `mod_settings.json` через `window.LumaCloudMod.*`
...
async function syncCompactPlayerStyle() {
  const enabled = (await window.LumaCloudMod.getStorageValue("compact-player/enabled")) === true;
...
window.LumaCloudMod.onStorageChanged((key: string) => {
...
export function CompactPlayer() {
  ...
  useEffect(() => {
    (async () => {
      setEnabled((await window.LumaCloudMod.getStorageValue("compact-player/enabled")) === true);
    })();
  }, []);
      ...
      <div className="flex items-center gap-3 px-3 pt-2">
        <Switch
          ...
          onCheckedChange={(value) => {
            setEnabled(value);
            window.LumaCloudMod.setStorageValue("compact-player/enabled", value);
          }}
        />
...
2. Скрипт сам скомпилирует и скопирует renderer из `src/mod/dist/*` в `.versions/<версия>/mod/app/LumaCloudMod/`
...
```js
const modPath = ".versions/5.86.0/mod/app/LumaCloudMod/";
```
...
```js
electron.ipcMain.handle("LumaCloudMod.system.getPlatform", () => process.platform);
```

`src/mod/preload.ts` (добавить в `exposeInMainWorld`):

```ts
getPlatform: () => electron.ipcRenderer.invoke("LumaCloudMod.system.getPlatform"),
```
...
```ts
const platform = await window.LumaCloudMod.getPlatform();
